...
public class CDictionaryBundle {
	...
	public void writeAllDicts(DataOutput out, int outBaseOffset) throws IOException{
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		DataOutput dataOutput = new DataOutputStream(baos);
		
		int offset = outBaseOffset + 4 * bundle.size();
		
		//System.out.println("DataBlock base offset:" + offset);
		
		for (int i = 0; i < bundle.size(); i++){
			bundle.get(i).write(dataOutput);
			out.writeInt(baos.size() + offset);
			//System.out.println("No. " + i + ", offset: " + (baos.size() + offset));
		}
