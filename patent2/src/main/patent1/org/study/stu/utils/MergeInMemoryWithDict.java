...
import org.study.stu.stu.TreeRecord;
...
public class MergeInMemoryWithDict {
	 ...
	 * 一般重复率高的归并顺序靠后
	 */
	public MergeInMemoryWithDict(GroupBundle groupBundle){		
		this(groupBundle, Constant.DEFAULT_SEPARATOR);
	}
	
	public MergeInMemoryWithDict(GroupBundle groupBundle, String groupSeparator){		
		this.groupBundle = groupBundle;		
		dictionaryBundle = new CDictionaryBundle(groupBundle.getFieldAmount());
	}
	
	public DataBlock mergeInMemory(Iterator<String> iter, int recordNumber){
	...
	private void initRecords(Iterator<String> iter, int recordNumber){
